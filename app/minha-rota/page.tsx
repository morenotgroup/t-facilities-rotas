...
export default function MinhaRotaPage() {
  ...
  const registrarLimpeza = (itemId: string, ambienteNome: string) => {
    const status = statusByItem[itemId] || 'LIMPO'
    const obs = obsByItem[itemId] || ''
    // Fase 1: ainda demo. Na Fase 2 vamos mandar isso para a API/BD.
    alert(
      `Registro DEMO:\n\nSala: ${ambienteNome}\nStatus: ${status}\nObs: ${
        obs || '—'
      }\nFoto: ${photoPreviewByItem[itemId] ? 'selecionada' : 'nenhuma'}`,
    )
  }

  return (
    <div className="flex flex-col gap-4 p-4 pb-6">
      {/* Bloco de entrada (glass) */}
      <section className="rounded-2xl border border-white/15 bg-white/10 p-4 text-xs text-slate-50 shadow-lg shadow-purple-900/40 backdrop-blur-xl">
        ...
        <p className="mt-1 text-[11px] text-slate-200/90">
          Use o e-mail corporativo para carregar a rota de limpeza do dia.
        </p>
        <div className="mt-3 flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={() => {}}
            className="hidden rounded-xl bg-gradient-to-br from-amber-400 via-orange-400 to-fuchsia-500 px-3 py-2 text-[11px] font-semibold text-slate-950 shadow-lg shadow-amber-500/40 md:block"
          >
            Atualizar
          </button>
        </div>
        <p className="mt-2 text-[10px] text-slate-300/80">
          (Por enquanto a rota muda automaticamente conforme o e-mail digitado.)
        </p>
      </section>
          ...
          <section className="rounded-2xl border border-white/15 bg-white/10 p-4 text-xs text-slate-50 shadow-lg shadow-purple-900/40 backdrop-blur-xl">
            <div className="flex items-start justify-between gap-2">
              <div>
                ...
                <p className="text-sm font-semibold">{rota.colaboradorNome}</p>
                <p className="text-[11px] text-slate-300">
                  {rota.colaboradorEmail}
                </p>
              </div>
              <div className="rounded-xl bg-black/40 px-3 py-1 text-[10px] font-medium text-amber-100 shadow-inner shadow-black/60">
                Turno: <span className="font-semibold">{rota.turno}</span>
              </div>
            </div>
            {rota.obsGeral && (
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        ...
                        <p className="text-[11px] text-slate-300">
                          Ordem {item.ordem} • {ambiente.andar} • {ambiente.bloco}
                        </p>
                      </div>
                      ...
                      <div className="mt-3 space-y-3">
                        ...
                        <div className="space-y-1">
                          ...
                          <div className="flex items-center gap-2">
                            <label
                              className="inline-flex cursor-pointer items-center gap-2 rounded-full border border-white/20 bg-black/30 px-3 py-1.5 text-[11px] font-medium text-slate-100 shadow-inner shadow-black/50 hover:border-amber-300/60 hover:text-amber-100"
                            >
                        ...
                        <button
                          type="button"
                          onClick={() => registrarLimpeza(item.id, ambiente.nome)}
